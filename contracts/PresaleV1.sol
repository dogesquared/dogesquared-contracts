    ...
    PausableUpgradeable
{
    ...
    /// @dev How many tokens the user has claimed via signature.
    ...
    IStakingManager public stakingManagerInterface;
    ...
    function initialize(
        ...
        baseDecimals = (10 ** 18);
        aggregatorInterface = IAggregator(_oracle);
    ...
    function setSplits(
        ...
        require(_wallets.length == _percentages.length, "Parameters mismatch");
        delete wallets;
        delete percentages;
    ...
    function buyWithUSDT(
        ...
        if (stake) {
            ...
            uint256 tokenAmount = (amount * baseDecimals);
            if (releaseTokens) {
        ...
        returns (bool)
    {
        ...
        if (stake) {
            ...
            uint256 tokenAmount = (_amount * baseDecimals);
            if (releaseTokens) {
                bool success = IERC20Upgradeable(saleToken).transfer(
                    _user,
                    tokenAmount
                ...
                require(success, "Token transfer failed");
                emit TokensClaimed(_user, tokenAmount, block.timestamp);
            } else {
                userDeposits[_user] += tokenAmount;
            }
        }
        usdRaised += usdPrice;
        splitETHValue(ethAmount);
     ...
     * @param _releaseTokens Permission to release tokens during presale and before the start of the claim.
    ...
    function setReleaseTokensStatus(bool _status) external onlyOwner {
        emit ReleaseTokensUpdated(releaseTokens, _status, block.timestamp);
        releaseTokens = _status;
    }
    ...
    /**
     * @dev Claim tokens using signature.
     * @param  tokensToRelease cannot be less than userClaimed.
     * @param signature The signature provided by the server.
     * @param stake Staking indication.
     */
    function claimWithSign(
        ...
        require(saleToken != address(0), "Sale token not added");
        require(!isBlacklisted[_msgSender()], "This Address is Blacklisted");
        _checkSignature(_msgSender(), tokensToRelease, signature);

        uint256 extraTokens = 0;
        uint256 claimed = userClaimed[_msgSender()];
        if (tokensToRelease > claimed) {
            extraTokens = tokensToRelease - claimed;
            userClaimed[_msgSender()] = tokensToRelease;
        }

        uint256 amount = userDeposits[_msgSender()] + extraTokens;
        require(amount > 0, "Nothing to claim");
        delete userDeposits[_msgSender()];
     ...
     * @param tokensToRelease The amount of tokens to release.
     * @param signature The signature provided by the server.
    ...
    function checkSignature(
        ...
        if (
            _signatureWallet(chainId, user, tokensToRelease, signature) !=
            signer
        ) {
            return false;
        }
    ...
    function _signatureWallet(
        ...
        return
            ECDSAUpgradeable.recover(
                keccak256(abi.encode(chainId_, user, tokensToRelease)),
                signature
            );
    }
    ...
    function updateFromBSC(
        ...
        require(_users.length == _userDeposits.length, "Parameters mismatch");
